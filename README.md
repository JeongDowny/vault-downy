# vault-downy — Claude Code 개인 장기 메모리 플러그인
...
```bash
mkdir -p ~/vault-downy/notes
```

기존 `~/vault-downy/` 가 있으면 그대로 사용합니다(노트 포맷 호환).
...
| 변수 | 기본값 | 설명 |
|------|--------|------|
| `VAULT_DOWNY_VAULT` | `~/vault-downy` | vault 루트 경로 |
| `VAULT_DOWNY_OLLAMA_URL` | `http://localhost:11434` | Ollama 서버 URL |
| `VAULT_DOWNY_EMBED_MODEL` | `bge-m3` | 임베딩 모델 |
| `VAULT_DOWNY_EXTRACT_MODEL` | `qwen2.5:7b` | 추출 모델 |
| `VAULT_DOWNY_TOP_K` | `3` | 인출 결과 수 |
...
- 노트는 `~/vault-downy/` 에 저장되며, 이 디렉터리를 private repo로 관리하길 권장합니다.
- secret 스캔이 활성화되어 있어, API 키·주민번호·JWT 등이 감지되면 자동으로 redact 후 저장합니다.
- 캡처된 후보 파일(`~/vault-downy/.derived/candidates.jsonl`)은 권한 600(소유자만 읽기)으로 생성됩니다.

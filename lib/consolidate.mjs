...
import { logCapture } from "./metrics.mjs";
...
export async function run({ extractFn = extractNotes, embedFn = _embed } = {}) {
  return withIndexLock(async () => {
    ...
    for (const nt of notes) {
      ...
      // 파일명/id = YYYYMMDD-<gist슬러그> (Obsidian 가독 + 멱등). 슬러그 충돌 시 해시 suffix.
      const date = new Date().toISOString().slice(0, 10).replace(/-/g, "");
      let id = `${date}-${slugify(g)}`;
      if (existsSync(join(notesDir(), `${id}.md`)))
        id = `${id}-${createHash("md5").update(g).digest("hex").slice(0, 4)}`;
      const tags = JSON.stringify((nt.tags || []).map(String));
      // gist+verbatim 기반 content_hash — 동일 내용 중복 감지용
      const content_hash = createHash("sha256").update(g + (verb || "")).digest("hex").slice(0, 16);
    ...
    if (written.length) {
      ...
      logCapture({ session, written, dedup_skipped: notes.length - written.length });
      try {
